...
namespace A03Function {
    ...
    function  calcTax(status: string, amount: number, person: number): number | undefined{
        if(status === 'NY') {
    ...
    let calcTax2:(status: string, amount: number, person: number) => number | undefined =
    function  calcTax(status: string, amount: number, person: number): number | undefined{
        if(status === 'NY') {
            return amount * 0.06 - person * 500;
        } else if (status === 'NJ') {
    ...
    function  calcTax3(amount: number, person: number = 2, status: string = 'NY'): number | undefined{
        if(status === 'NY') {
            return amount * 0.06 - person * 500;
        } else if (status === 'NJ') {
            return amount * 0.03 - person * 300;
    ...
    function  calcTax4(amount: number, person: number = 2, status?: string): number | undefined{
